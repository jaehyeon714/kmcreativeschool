...
public class ContactController {
	...
	// 문의 목록 조회
	@GetMapping("/contact")
	public String showContactList(Model model, @RequestParam(	defaultValue = "1") int cPage,
	        @RequestParam(defaultValue = "7") int numPerPage) {
	    List<Contact> contacts = qnaService.selectAllBoard(); 
	    long totalData = contacts.size(); 
	    List<Contact> paginatedContacts = contacts.stream()
	...
	// 문의 저장
	/*
	 * @PostMapping("/contactwriteForm") public String saveContact(@RequestParam
	 * String title,
	 * 
	 * @RequestParam String contactContent) { Contact contact = Contact.builder()
	 * .seq((contacts.size() + 1)) // ID는 리스트의 크기를 기반으로 설정 .title(title)
	 * .contactContent(contactContent) .dateTime(null) .build();
	 * contacts.add(contact); // 문의를 리스트에 저장 return
	 * "redirect:/contact/contactview?seq=" + contact.getSeq(); // 저장 후 상세보기로 리다이렉트
	 * }
	 */
